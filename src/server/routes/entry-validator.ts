...
type FieldTypeValidator = (slug: string, value: unknown) => string | null
...
const fieldTypeValidators: Record<FieldType, FieldTypeValidator> = {
  ...
  boolean: (slug, value) => {
    if (typeof value !== "boolean") return `フィールド「${slug}」は真偽値である必要があります`
    return null
  },
}

const checkFieldType = (slug: string, type: FieldType, value: unknown): string | null =>
  fieldTypeValidators[type](slug, value)
